...
filered_leg1flights = FILTER filered_leg1flights BY (Origin == 'ORD') AND (Dest != 'JFK') AND (Cancelled != 1)  AND (Diverted != 1) AND ((Year == 2007 AND Month >= 06) OR (Year == 2008 AND Month <= 05));

filered_leg2flights = FILTER filered_leg2flights BY (Origin != 'ORD') AND (Dest == 'JFK') AND (Cancelled != 1)  AND (Diverted != 1) AND ((Year == 2007 AND Month >= 06) OR (Year == 2008 AND Month <= 05)  );
...
delays = FOREACH flightsjoinedandfilteredbytime GENERATE filered_leg1flights::ArrDelayMinutes+filered_leg2flights::ArrDelayMinutes;
